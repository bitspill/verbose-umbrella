...
func onMpCompleted() {
	...
	results, err := datastore.Client().Search(datastore.Index(oip042DeactivateIndex)).Query(q).Size(10000).Sort("meta.time", false).Do(context.TODO())
	if err != nil {
	...
	for _, v := range results.Hits.Hits {
		...
		up := elastic.NewBulkUpdateRequest().Index(datastore.Index(oip042ArtifactIndex)).Id(ea.Deactivate.Reference).Script(s)
		...
		up = elastic.NewBulkUpdateRequest().Index(datastore.Index(oip042DeactivateIndex)).Id(ea.Meta.Txid).Script(s)
		datastore.AutoBulk.Add(up)
	}
}
